        ...
        <div className="mx-[100px]">
          ...
          <div className="flex relative text-center justify-center my-5">
            <div className="flex space-x-10 mt-10 text-primary font-Sans">
              {["City Name", "City Name", "City Name", "City Name", "City Name", "City Name", "City Name"].map((city, index) => (
                <div key={index} className="border rounded-full p-1 px-5 bg-white">
                  {city}
                </div>
              ))}
            </div>
          ...
          {exclusivDatas.map((offer, id) => (
            <div key={id} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-1 justify-center my-10 border rounded-lg relative">
              <div className="bg-white shadow-md overflow-hidden rounded-l-lg">
                <Image src={yachtExclusive} alt="YACHT" className="w-full h-60 object-cover" />
              </div>
              <div className="bg-white shadow-md overflow-hidden">
                <Image src={helicoptarExclusive} alt="HELICOPTER" className="w-full h-60 object-cover" />
              </div>
              <div className="bg-white shadow-md rounded-r-lg overflow-hidden">
                <Image src={fastYachts} alt="JETSKI" className="w-full h-60 object-cover" />
              </div>
              ...
              <div className="absolute bottom-5 left-3 space-y-2 mx-5">
                ...
                <p className="font-Sans text-gray-200 text-xs max-w-[600px]">{offer.deals}</p>
              </div>
  
              <div className="absolute bottom-5 right-3">
                <div className="flex bg-white bg-opacity-50 rounded-full px-3 space-x-5">
                  <div className="flex space-x-1">
                    <h1 className="font-bold font-Sans text-white">{offer.priceAfter}</h1>
                    <span className="text-xs mt-2 text-white font-bold z-40 font-Sans">AED</span>
                  </div>
                  <div className="line-through text-white font-Sans">{offer.priceBefore} AED</div>
                </div>
